  ...
  "inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-md text-sm font-medium transition-all disabled:pointer-events-none disabled:opacity-50 [&_svg]:pointer-events-none shrink-0 [&_svg]:shrink-0 outline-none focus-visible:border-ring focus-visible:ring-ring/50 focus-visible:ring-[3px] aria-invalid:ring-destructive/20 dark:aria-invalid:ring-destructive/40 aria-invalid:border-destructive",
  {
    variants: {
      variant: {
        ...
        appGreenBtn:
          'bg-[#BFE831] text-black font-[Manrope] font-medium text-[18px] leading-[26px] capitalize cursor-pointer transition-all duration-200',
        appBlueBtn:
          'bg-[#378EF2] text-white font-[Manrope] font-medium text-[18px] leading-[26px] capitalize cursor-pointer transition-all duration-200',
      },
      effect: {
      ...
      size: {
        ...
        'icon-sm': 'size-8',
        'icon-lg': 'size-10',
        pill: 'h-[54px] rounded-[50px] px-[30px]',
      },
    },
    defaultVariants: {
      variant: 'default',
    ...
    return (
      ...
      >
         {Icon &&
           iconPlacement === 'left' &&
           (effect === 'expandIcon' ? (
             <div className="w-0 translate-x-[0%] pr-0 opacity-0 transition-all duration-200 group-hover:w-5 group-hover:translate-x-100 group-hover:pr-3 group-hover:opacity-100">
               <div 
                 className="p-1 rounded-full"
                 style={{ backgroundColor: iconBg }}
               >
                 <Icon stroke={iconStroke} className="size-4"/>
               </div>
             </div>
           ) : (
             <div 
        ...
        {Icon &&
          iconPlacement === 'right' &&
          (effect === 'expandIcon' ? (
             <div className="w-0 translate-x-full pl-0 opacity-0 rounded-full flex items-center justify-center transition-all duration-200 group-hover:w-5 group-hover:translate-x-0 group-hover:pl-3 group-hover:opacity-100">
               <div 
                 className="p-1 rounded-full"
                 style={{ backgroundColor: iconBg }}
               >
